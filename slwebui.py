...
from spiderlab import __version__
from spiderlab.logger import logListenerSetup, logWorkerSetup

mp.set_start_method("spawn", force=True)


class SpiderLabWebUi:
    """SpiderLab web interface."""
    ...
    def __init__(self: 'SpiderLabWebUi', web_config: dict, config: dict, loggingQueue: 'logging.handlers.QueueListener' = None) -> None:
        """Initialize web server.

        Args:
            web_config (dict): config settings for web interface (interface, port, root path)
            config (dict): SpiderLab config
            loggingQueue: TBD
        ...
        self.log = logging.getLogger(f"spiderlab.{__name__}")
    ...
    def error_page(self: 'SpiderLabWebUi') -> None:
        """Error page."""
    ...
    def error_page_401(self: 'SpiderLabWebUi', status: str, message: str, traceback: str, version: str) -> str:
    ...
    def error_page_404(self: 'SpiderLabWebUi', status: str, message: str, traceback: str, version: str) -> str:
        ...
        """
        templ = Template(filename='spiderlab/templates/error.tmpl', lookup=self.lookup)
        return templ.render(message='Not Found', docroot=self.docroot, status=status, version=__version__)

    def jsonify_error(self: 'SpiderLabWebUi', status: str, message: str) -> dict:
    ...
    def error(self: 'SpiderLabWebUi', message: str) -> None:
    ...
    def cleanUserInput(self: 'SpiderLabWebUi', inputList: list) -> list:
    ...
    def searchBase(self: 'SpiderLabWebUi', id: str = None, eventType: str = None, value: str = None) -> list:
        """Search.
    ...
    def buildExcel(self: 'SpiderLabWebUi', data: list, columnNames: list, sheetNameIndex: int = 0) -> str:
    ...
    @cherrypy.expose
    def scanexportlogs(self: 'SpiderLabWebUi', id: str, dialect: str = "excel") -> bytes:
        """Get scan log
        ...
        cherrypy.response.headers['Content-Disposition'] = f"attachment; filename=SpiderLab-{id}.log.csv"
        cherrypy.response.headers['Content-Type'] = "application/csv"
        cherrypy.response.headers['Pragma'] = "no-cache"
        return fileobj.getvalue().encode('utf-8')

    @cherrypy.expose
    def scancorrelationsexport(self: 'SpiderLabWebUi', id: str, filetype: str = "csv", dialect: str = "excel") -> str:
        ...
        if filetype.lower() in ["xlsx", "excel"]:
            ...
            if scan_name:
                fname = f"{scan_name}-SpiderLab-correlations.xlxs"
            else:
                fname = "SpiderLab-correlations.xlxs"
        ...
        if filetype.lower() == 'csv':
            ...
            if scan_name:
                fname = f"{scan_name}-SpiderLab-correlations.csv"
            else:
                fname = "SpiderLab-correlations.csv"
    ...
    @cherrypy.expose
    def scaneventresultexport(self: 'SpiderLabWebUi', id: str, type: str, filetype: str = "csv", dialect: str = "excel") -> str:
    ...
    @cherrypy.expose
    def scaneventresultexportmulti(self: 'SpiderLabWebUi', ids: str, filetype: str = "csv", dialect: str = "excel") -> str:
        ...
        if filetype.lower() in ["xlsx", "excel"]:
            ...
            if len(ids.split(',')) > 1 or scan_name == "":
                ...
                fname = scan_name + "-SpiderLab.xlsx"
    ...
    @cherrypy.expose
    def scansearchresultexport(self: 'SpiderLabWebUi', id: str, eventType: str = None, value: str = None, filetype: str = "csv", dialect: str = "excel") -> str:
    ...
    @cherrypy.expose
    def scanexportjsonmulti(self: 'SpiderLabWebUi', ids: str) -> str:
        ...
        if len(ids.split(',')) > 1 or scan_name == "":
            fname = "SpiderLab.json"
        else:
            fname = scan_name + "-SpiderLab.json"
    ...
    @cherrypy.expose
    def scanviz(self: 'SpiderLabWebUi', id: str, gexf: str = "0") -> str:
        ...
        if gexf == "0":
            return SpiderLabHelpers.buildGraphJson([root], data)

        if not scan_name:
            fname = "SpiderLab.gexf"
        else:
            fname = scan_name + "SpiderLab.gexf"

        cherrypy.response.headers['Content-Disposition'] = f"attachment; filename={fname}"
        cherrypy.response.headers['Content-Type'] = "application/gexf"
        cherrypy.response.headers['Pragma'] = "no-cache"
        return SpiderLabHelpers.buildGraphGexf([root], "SpiderLab Export", data)

    @cherrypy.expose
    def scanvizmulti(self: 'SpiderLabWebUi', ids: str, gexf: str = "1") -> str:
        ...
        cherrypy.response.headers['Content-Type'] = "application/gexf"
        cherrypy.response.headers['Pragma'] = "no-cache"
        return SpiderLabHelpers.buildGraphGexf(roots, "SpiderLab Export", data)

    @cherrypy.expose
    @cherrypy.tools.json_out()
    def scanopts(self: 'SpiderLabWebUi', id: str) -> dict:
    ...
    @cherrypy.expose
    def rerunscan(self: 'SpiderLabWebUi', id: str) -> None:
        """Rerun a scan.
        ...
        if not targetType:
            ...
            # target type again.
            targetType = SpiderLabHelpers.targetTypeFromString(f'"{scantarget}"')

        if targetType not in ["HUMAN_NAME", "BITCOIN_ADDRESS"]:
            scantarget = scantarget.lower()

        # Start running a new scan
        scanId = SpiderLabHelpers.genScanInstanceId()
        try:
            p = mp.Process(target=startSpiderLabScanner, args=(self.loggingQueue, scanname, scanId, scantarget, targetType, modlist, cfg))
            p.daemon = True
            p.start()
    ...
    @cherrypy.expose
    def rerunscanmulti(self: 'SpiderLabWebUi', ids: str) -> str:
        """Rerun scans.
        ...
        for id in ids.split(","):
            ...
            if targetType is None:
                ...
                return self.error("Invalid target type. Could not recognize it as a target SpiderLab supports.")
        ...
        templ = Template(filename='spiderlab/templates/scanlist.tmpl', lookup=self.lookup)
        return templ.render(rerunscans=True, docroot=self.docroot, pageid="SCANLIST", version=__version__)

    @cherrypy.expose
    def newscan(self: 'SpiderLabWebUi') -> str:
        ...
        templ = Template(filename='spiderlab/templates/newscan.tmpl', lookup=self.lookup)
        return templ.render(pageid='NEWSCAN', types=types, docroot=self.docroot,
                            modules=self.config['__modules__'], scanname="",
                            selectedmods="", scantarget="", version=__version__)

    @cherrypy.expose
    def clonescan(self: 'SpiderLabWebUi', id: str) -> str:
    ...
    @cherrypy.expose
    def index(self: 'SpiderLabWebUi') -> str:
        """Show scan list page.
    ...
    @cherrypy.expose
    def scaninfo(self: 'SpiderLabWebUi', id: str) -> str:
        ...
        templ = Template(filename='spiderlab/templates/scaninfo.tmpl', lookup=self.lookup, input_encoding='utf-8')
        return templ.render(id=id, name=html.escape(res[0]), status=res[5], docroot=self.docroot, version=__version__,
                            pageid="SCANLIST")

    @cherrypy.expose
    def opts(self: 'SpiderLabWebUi', updated: str = None) -> str:
        ...
        """
        templ = Template(filename='spiderlab/templates/opts.tmpl', lookup=self.lookup)
    ...
    @cherrypy.expose
    def optsexport(self: 'SpiderLabWebUi', pattern: str = None) -> str:
        ...
        cherrypy.response.headers['Content-Disposition'] = 'attachment; filename="SpiderLab.cfg"'
        cherrypy.response.headers['Content-Type'] = "text/plain"
        return content

    @cherrypy.expose
    @cherrypy.tools.json_out()
    def optsraw(self: 'SpiderLabWebUi') -> str:
    ...
    @cherrypy.expose
    @cherrypy.tools.json_out()
    def scandelete(self: 'SpiderLabWebUi', id: str) -> str:
        """Delete scan(s).
    ...
    @cherrypy.expose
    def savesettings(self: 'SpiderLabWebUi', allopts: str, token: str, configFile: 'cherrypy._cpreqbody.Part' = None) -> None:
        ...
        if configFile and configFile.file:
            try:
                ...
                return self.error(f"Failed to parse input file. Was it generated from SpiderLab? ({e})")
    ...
    @cherrypy.expose
    def savesettingsraw(self: 'SpiderLabWebUi', allopts: str, token: str) -> str:
        """Save settings, also used to completely reset them to default.
    ...
    def reset_settings(self: 'SpiderLabWebUi') -> bool:
    ...
    @cherrypy.expose
    def resultsetfp(self: 'SpiderLabWebUi', id: str, resultids: str, fp: str) -> str:
    ...
    @cherrypy.expose
    @cherrypy.tools.json_out()
    def eventtypes(self: 'SpiderLabWebUi') -> list:
    ...
    @cherrypy.expose
    @cherrypy.tools.json_out()
    def modules(self: 'SpiderLabWebUi') -> list:
        """List all modules.
    ...
    @cherrypy.expose
    @cherrypy.tools.json_out()
    def correlationrules(self: 'SpiderLabWebUi') -> list:
    ...
    @cherrypy.expose
    @cherrypy.tools.json_out()
    def ping(self: 'SpiderLabWebUi') -> list:
        """For the CLI to test connectivity to this server.

        Returns:
            list: SpiderLab version as JSON
        """
        return ["SUCCESS", __version__]

    @cherrypy.expose
    @cherrypy.tools.json_out()
    def query(self: 'SpiderLabWebUi', query: str) -> str:
    ...
    @cherrypy.expose
    def startscan(self: 'SpiderLabWebUi', scanname: str, scantarget: str, modulelist: str, typelist: str, usecase: str) -> str:
        """Initiate a scan.
    ...
    @cherrypy.expose
    @cherrypy.tools.json_out()
    def stopscan(self: 'SpiderLabWebUi', id: str) -> str:
        """Stop a scan.
        ...
        for scan_id in ids:
            ...
            if scan_status != "RUNNING" and scan_status != "STARTING":
                return self.jsonify_error('400', f"The running scan is currently in the state '{scan_status}', please try again later or restart SpiderLab.")
    ...
    @cherrypy.expose
    @cherrypy.tools.json_out()
    def scanlog(self: 'SpiderLabWebUi', id: str, limit: str = None, rowId: str = None, reverse: str = None) -> list:
        """Scan log data.
    ...
    @cherrypy.expose
    @cherrypy.tools.json_out()
    def scanerrors(self: 'SpiderLabWebUi', id: str, limit: str = None) -> list:
        """Scan error data.
    ...
    @cherrypy.expose
    @cherrypy.tools.json_out()
    def scanlist(self: 'SpiderLabWebUi') -> list:
    ...
    @cherrypy.expose
    @cherrypy.tools.json_out()
    def scanstatus(self: 'SpiderLabWebUi', id: str) -> list:
    ...
    @cherrypy.expose
    @cherrypy.tools.json_out()
    def scansummary(self: 'SpiderLabWebUi', id: str, by: str) -> list:
    ...
    @cherrypy.expose
    @cherrypy.tools.json_out()
    def scancorrelations(self: 'SpiderLabWebUi', id: str) -> list:
    ...
    @cherrypy.expose
    @cherrypy.tools.json_out()
    def scaneventresults(self: 'SpiderLabWebUi', id: str, eventType: str = None, filterfp: bool = False, correlationId: str = None) -> list:
    ...
    @cherrypy.expose
    @cherrypy.tools.json_out()
    def scaneventresultsunique(self: 'SpiderLabWebUi', id: str, eventType: str, filterfp: bool = False) -> list:
    ...
    @cherrypy.expose
    @cherrypy.tools.json_out()
    def search(self: 'SpiderLabWebUi', id: str = None, eventType: str = None, value: str = None) -> list:
        """Search scans.
    ...
    @cherrypy.expose
    @cherrypy.tools.json_out()
    def scanhistory(self: 'SpiderLabWebUi', id: str) -> list:
    ...
    @cherrypy.expose
    @cherrypy.tools.json_out()
    def scanelementtypediscovery(self: 'SpiderLabWebUi', id: str, eventType: str) -> dict:
        ...
        del pc['ROOT']
        retdata['tree'] = SpiderLabHelpers.dataParentChildToTree(pc)
