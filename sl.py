...
from slscan import startSpiderLabScanner
from slwebui import SpiderLabWebUi
from spiderlab import SpiderLabHelpers
from spiderlab import SpiderLabDb
from spiderlab import SpiderLabCorrelator
...
def main() -> None:
    ...
    slConfig = {
        ...
        '_genericusers': ",".join(SpiderLabHelpers.usernamesFromWordlists(['generic-usernames'])),
        '__database': f"{SpiderLabHelpers.dataPath()}/spiderlab.db",
    ...
    args = None
    p = argparse.ArgumentParser(description=f"SpiderLab {__version__}: Open Source Intelligence Automation.")
    ...
    p.add_argument("-V", "--version", action='store_true', help="Display the version of SpiderLab and exit.")
    p.add_argument("-max-threads", type=int, help="Max number of modules to run concurrently.")
    args = p.parse_args()

    if args.version:
        print(f"SpiderLab {__version__}: Open Source Intelligence Automation.")
        sys.exit(0)

    if args.max_threads:
        slConfig['_maxthreads'] = args.max_threads

    if args.debug:
        slConfig['_debug'] = True
    else:
        slConfig['_debug'] = False

    if args.q:
        slConfig['__logging'] = False

    loggingQueue = mp.Queue()
    logListenerSetup(loggingQueue, slConfig)
    logWorkerSetup(loggingQueue)
    log = logging.getLogger(f"spiderlab.{__name__}")

    # Add descriptions of the global config options
    slConfig['__globaloptdescs__'] = slOptdescs

    # Load each module in the modules directory with a .py extension
    try:
        mod_dir = os.path.dirname(os.path.abspath(__file__)) + '/modules/'
        slModules = SpiderLabHelpers.loadModulesAsDict(mod_dir, ['slp_template.py'])
    ...
    try:
        ...
        correlationRulesRaw = SpiderLabHelpers.loadCorrelationRulesRaw(correlations_dir, ['template.yaml'])
    except BaseException as e:
    ...
    try:
        dbh = SpiderLabDb(slConfig)
    except Exception as e:
        log.critical(f"Failed to initialize database: {e}", exc_info=True)
        sys.exit(-1)

    # Sanity-check the rules and parse them
    slCorrelationRules = list()
    if not correlationRulesRaw:
        log.error(f"No correlation rules found in correlations directory: {correlations_dir}")
    else:
        try:
            correlator = SpiderLabCorrelator(dbh, correlationRulesRaw)
            slCorrelationRules = correlator.get_ruleset()
        except Exception as e:
            log.critical(f"Failure initializing correlation rules: {e}", exc_info=True)
            sys.exit(-1)

    # Add modules and correlation rules to slConfig so they can be used elsewhere
    slConfig['__modules__'] = slModules
    slConfig['__correlationrules__'] = slCorrelationRules
    ...
    if args.correlate:
        ...
        try:
            ...
            corr = SpiderLabCorrelator(dbh, correlationRulesRaw, args.correlate)
    ...
    if args.modules:
        ...
        for m in sorted(slModules.keys()):
            if "__" in m:
                continue
            print(f"{m.ljust(25)}  {slModules[m]['descr']}")
        sys.exit(0)

    if args.types:
        dbh = SpiderLabDb(slConfig, init=True)
    ...
    if args.l:
        ...
        slWebUiConfig['host'] = host
        slWebUiConfig['port'] = port

        start_web_server(slWebUiConfig, slConfig, loggingQueue)
        sys.exit(0)

    start_scan(slConfig, slModules, args, loggingQueue)


def start_scan(slConfig: dict, slModules: dict, args, loggingQueue) -> None:
    """Start scan

    Args:
        slConfig (dict): SpiderLab config options
        slModules (dict): modules
        args (argparse.Namespace): command line args
        loggingQueue (Queue): main SpiderLab logging queue
    """
    log = logging.getLogger(f"spiderlab.{__name__}")
    ...
    if "." not in target and not target.startswith("+") and '"' not in target:
        target = f"\"{target}\""
    targetType = SpiderLabHelpers.targetTypeFromString(target)
    ...
    if args.t:
        types = args.t
        modlist = sl.modulesProducing(types)
    ...
    if args.u:
        ...
        for mod in slConfig['__modules__']:
            if usecase == 'All' or usecase in slConfig['__modules__'][mod]['group']:
                modlist.append(mod)

    # Add slp__stor_stdout to the module list
    typedata = dbh.eventTypes()
    types = dict()
    for r in typedata:
        types[r[1]] = r[0]

    slp__stor_stdout_opts = slConfig['__modules__']['slp__stor_stdout']['opts']
    slp__stor_stdout_opts['_eventtypes'] = types
    if args.f:
        if args.f and not args.t:
            log.error("You can only use -f with -t. Use --help for guidance.")
            sys.exit(-1)
        slp__stor_stdout_opts['_showonlyrequested'] = True
    if args.F:
        slp__stor_stdout_opts['_requested'] = args.F.split(",")
        slp__stor_stdout_opts['_showonlyrequested'] = True
    if args.o:
        if args.o not in ["tab", "csv", "json"]:
            log.error("Invalid output format selected. Must be 'tab', 'csv' or 'json'.")
            sys.exit(-1)
        slp__stor_stdout_opts['_format'] = args.o
    if args.t:
        slp__stor_stdout_opts['_requested'] = args.t.split(",")
    if args.n:
        slp__stor_stdout_opts['_stripnewline'] = True
    if args.r:
        slp__stor_stdout_opts['_showsource'] = True
    if args.S:
        slp__stor_stdout_opts['_maxlength'] = args.S
    if args.D:
        slp__stor_stdout_opts['_csvdelim'] = args.D
    if args.x:
        tmodlist = list()
        modlist = list()
        xmods = sl.modulesConsuming([targetType])
        for mod in xmods:
            if mod not in modlist:
                tmodlist.append(mod)
        ...
        for mod in tmodlist:
            for r in rtypes:
                if not slModules[mod]['provides']:
                    continue
                if r in slModules[mod].get('provides', []) and mod not in modlist:
                    modlist.append(mod)
    ...
    modlist += ["slp__stor_db", "slp__stor_stdout"]

    if slConfig['__logging']:
        log.info(f"Modules enabled ({len(modlist)}): {','.join(modlist)}")

    cfg = sl.configUnserialize(dbh.configGet(), slConfig)
    ...
    try:
        p = mp.Process(target=startSpiderLabScanner, args=(loggingQueue, scanName, scanId, target, targetType, modlist, cfg))
        p.daemon = True
        p.start()
    except BaseException as e:
...
def start_web_server(slWebUiConfig: dict, slConfig: dict, loggingQueue=None) -> None:
    """Start the web server so you can start looking at results

    Args:
        slWebUiConfig (dict): web server options
        slConfig (dict): SpiderLab config options
        loggingQueue (Queue): main SpiderLab logging queue
    """
    log = logging.getLogger(f"spiderlab.{__name__}")

    web_host = slWebUiConfig.get('host', '127.0.0.1')
    web_port = slWebUiConfig.get('port', 5001)
    web_root = slWebUiConfig.get('root', '/')
    cors_origins = slWebUiConfig.get('cors_origins', [])
    ...
    conf = {
        ...
        '/static': {
            ...
            'tools.staticdir.root': f"{os.path.dirname(os.path.abspath(__file__))}/spiderlab"
        }
    }

    secrets = dict()
    passwd_file = SpiderLabHelpers.dataPath() + '/passwd'
    ...
    if secrets:
        ...
        warn_msg += "Refer to https://www.spiderlab.net/documentation/#security.\n"
        warn_msg += "********************************************************************\n"
        log.warning(warn_msg)

    using_ssl = False
    key_path = SpiderLabHelpers.dataPath() + '/spiderlab.key'
    crt_path = SpiderLabHelpers.dataPath() + '/spiderlab.crt'
    ...
    print("")
    print("*************************************************************")
    print(" Use SpiderLab by starting your web browser of choice and ")
    ...
    cherrypy.quickstart(SpiderLabWebUi(slWebUiConfig, slConfig, loggingQueue), script_name=web_root, config=conf)
...
if __name__ == '__main__':
    if sys.version_info < (3, 7):
        print("SpiderLab requires Python 3.7 or higher.")
        sys.exit(-1)

    if len(sys.argv) <= 1:
        print("SpiderLab requires -l <ip>:<port> to start the web server. Try --help for guidance.")
        sys.exit(-1)

    # TODO: remove this after a few releases (added in 3.5 pre-release 2021-09-05)
    from pathlib import Path
    if os.path.exists('spiderlab.db'):
        print(f"ERROR: spiderlab.db file exists in {os.path.dirname(__file__)}")
        print("SpiderLab no longer supports loading the spiderlab.db database from the application directory.")
        print(f"The database is now loaded from your home directory: {Path.home()}/.spiderlab/spiderlab.db")
        print(f"This message will go away once you move or remove spiderlab.db from {os.path.dirname(__file__)}")
        sys.exit(-1)
    ...
    if os.path.exists('passwd'):
        ...
        print("SpiderLab no longer supports loading credentials from the application directory.")
        print(f"The passwd file is now loaded from your home directory: {Path.home()}/.spiderlab/passwd")
