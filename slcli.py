...
class SpiderLabCli(cmd.Cmd):
    ...
    # Make a request to the SpiderLab server
    def request(self, url, post=None):
        ...
        headers = {
            "User-agent": "SpiderLab-CLI/" + self.version,
    ...
    # SL> scans "blahblah test" | top 10 | grep foo ->
    ...
    def do_ping(self, line):
        """ping
        Ping the SpiderLab server to ensure it's responding."""
    ...
    # List all SpiderLab correlation rules
    ...
    # List all SpiderLab data element types.
    ...
    def do_load(self, line):
        """load <file>
        Execute SpiderLab CLI commands found in <file>."""
        pass
    ...
    def do_data(self, line):
        ...
        d = d.replace("&lt;/SLURL&gt;", "").replace("&lt;SLURL&gt;", "")
        self.send_output(d, line, titles=titles)
    ...
    def print_topics(self, header, cmds, cmdlen, maxcol):
        ...
        helpmap = [
            ...
            ["exit", "Exit the SpiderLab CLI (won't impact running scans)."],
            ["ping", "Test connectivity to the SpiderLab server."],
            ...
            ["query", "Run SQL against the SpiderLab SQLite database."],
    ...
    def do_set(self, line):
        """set [opt [= <val>]]
        Set a configuration variable in SpiderLab."""
        ...
        if not d:
            self.edprint("Unable to obtain SpiderLab server-side config.")
            return
        ...
        j = json.loads(d)
        if j[0] == "ERROR":
            self.edprint("Error fetching SpiderLab server-side config.")
            return
        ...
        if val:
            # submit all non-CLI vars to the SL server
            confdata = dict()
            found = False
            ...
            if not d:
                self.edprint("Unable to set SpiderLab server-side config.")
                return

            j = json.loads(d)
            if j[0] == "ERROR":
                self.edprint(f"Error setting SpiderLab server-side config: {j[1]}")
                return
...
if __name__ == "__main__":
    p = argparse.ArgumentParser(description='SpiderLab: Open Source Intelligence Automation.')
    p.add_argument("-d", "--debug", help="Enable debug output.", action='store_true')
    p.add_argument("-s", metavar="URL", type=str, help="Connect to SpiderLab server on URL. By default, a connection to http://127.0.0.1:5001 will be attempted.")
    p.add_argument("-u", metavar="USER", type=str, help="Username to authenticate to SpiderLab server.")
    p.add_argument("-p", metavar="PASS", type=str, help="Password to authenticate to SpiderLab server. Consider using -P PASSFILE instead so that your password isn't visible in your shell history or in process lists!")
    p.add_argument("-P", metavar="PASSFILE", type=str, help="File containing password to authenticate to SpiderLab server. Ensure permissions on the file are set appropriately!")
    p.add_argument("-e", metavar="FILE", type=str, help="Execute commands from FILE.")
    p.add_argument("-l", metavar="FILE", type=str, help="Log command history to FILE. By default, history is stored to ~/.spiderlab_history unless disabled with -n.")
    ...
    if args.l:
        ...
        try:
            s.ownopts['cli.history_file'] = expanduser("~") + "/.spiderlab_history"
        except BaseException as e:
            s.dprint(f"Failed to set 'cli.history_file': {e}")
            s.dprint("Using '.spiderlab_history' in working directory")
            s.ownopts['cli.history_file'] = ".spiderlab_history"
    if args.o:
